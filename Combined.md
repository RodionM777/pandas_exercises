...
```python
dataset=pd.read_csv(r"D:\dataset1.txt",sep="|")
print(dataset.head())
```
...
```python
print(users["occupation"].value_counts(ascending=False).head(1))
```
...
print(age_counts[age_counts==min_age])
...
```python
data=pd.read_csv(r"D:\euroset.csv")
data.head(10)
```
...
```python
answer=discipline.groupby("Team")["Yellow Cards"].mean()
print(answer.mean())
```
...
```python
drinks=pd.read_csv(r"D:\drinks.csv")
```
...
drinks.groupby("continent").agg(np.mean)
```

    C:\Users\Rodion\AppData\Local\Temp\ipykernel_15192\251502483.py:1: FutureWarning: The provided callable <function mean at 0x00000220BF88D580> is currently using DataFrameGroupBy.mean. In a future version of pandas, the provided callable will be used directly. To keep current behavior pass the string "mean" instead.
      drinks.groupby("continent").agg(np.mean)
...
```python
print(drinks["spirit_servings"].agg(["mean","min","max"]))
```
